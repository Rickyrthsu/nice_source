...
# --- 【重點修改】Pornhub 爬蟲邏輯 (支援 Pornstar 與 Model) ---
def scrape_pornhub_actor(name, scraper):
    print(f"  [Pornhub] 正在搜尋: {name} ...")
    
    # 搜尋 URL (Pornhub 的 pornstars 搜尋通常也包含 models)
    ...
    try:
        ...
        if result_container:
            ...
            if first_item:
                # 【關鍵修改】同時接受 '/pornstar/' 和 '/model/'
                link_tag = first_item.find('a', href=re.compile(r'/(pornstar|model)/'))
        ...
        if not target_profile_url:
            print(f"  [Pornhub] 搜尋結果為空，或是找不到符合的演員/模特: {name}")
            return None
